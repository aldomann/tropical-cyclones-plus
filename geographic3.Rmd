...
plot_clusters <- function(basin.name, type, min.speed = 0, n.clust = 2) {
	...
	if (type == "first") {
		mat <- storms.small %>% select(first.lat, first.long, distance)
	} else if (type == "last") {
		mat <- storms.small %>% select(last.lat, last.long, distance)
	}
	
	clust <- hclust(dist(mat), method = "complete")
	tree <- cutree(clust, n.clust)
	
	gg <- ggplot(as_tibble(cbind(storms.small, clust = as.factor(tree))) ) +
